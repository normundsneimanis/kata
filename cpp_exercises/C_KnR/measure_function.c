...
typedef int (*searchFunctionPtr)(int, int[], int);
int measurefunction(searchFunctionPtr searchFunction);
int my_function(searchFunctionPtr searchFunction);


int main() {
	measurefunction(binsearch);
	measurefunction(binsearchOrig);
...
int my_function(searchFunctionPtr searchFunction) {
    ...
    ok = nok = 0;
    for (i = 0; i < ARRAY_MAX; i++) {
        if ((result = searchFunction(i, array, ARRAY_MAX)) == i) {
            ok++;
        } else {
            nok++;
        }
